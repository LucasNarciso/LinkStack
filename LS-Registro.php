...
<meta charset="UTF-8">
    ...
    <title>Link Stack - Registro</title>
    <script>

        //Função que habilita/desabilita o botão de registrar através do checkbox dos termos
        function termos(){
            botao = document.getElementById('botaoReg');
            checkbox = document.getElementById('checkTermos');

            if (checkbox.checked == false) {
                checkbox.checked = false;
                botao.disabled = true;
                botao.classList.add('disable');
                botao.classList.remove('botaoRegistrarActive');
            }else{
                checkbox.checked = true;
                botao.disabled = false;
                botao.classList.remove('disable');
                botao.classList.add('botaoRegistrarActive');
            }
            
        }

        function showInfoPass() {
            document.getElementById('infoPass').style.visibility = "visible";
        }

        function hiddeInfoPass(){
            document.getElementById('infoPass').style.visibility = "hidden";
        }
        ...
        function hiddeInfoUser(){
            document.getElementById('infoUser').style.visibility = "hidden";
        }

        //Função que analiza e mostra ao usuário o erro ocorrido, caso haja erro.
        function atualizaErro() {
            ...
            try {
                ...
                mensagem.innerHTML = document.getElementById('inputErroRegistro').value;
...
<body onload="termos(), atualizaErro()" align="center">
    
    <!-- ÁREA DO PHP -->

    <?php
        include_once('LS-Sistema.php');
        chave($_POST['inputKey'], $pdo);
    ...
    <!-- ÁREA DA JANELA DE Registro -->
    <form action="LS-Registro.php" method="post">
        <div class="divSeparacao">
            <div class="divRegistro" align="center" style="box-shadow: 3px 3px 10px black;">

                <p style="text-align: center; font-size: 1.8em;">Registro de Membro</p>

                <div class="divInputsRegistro" align="center">
                    <p>Usuário:</p>
                    <div align="center">
                        <?php
                            if (isset($_POST["registrar"])) {
                                echo"<input type=text name='NomeUsuario' value='".$_POST["NomeUsuario"]."' style='text-align:center;'></input>";
                            }else {
                                echo"<input type=text name='NomeUsuario' style='text-align:center;'></input>";
                            }

                        ?>
                        
                        <svg class="infoUser" onmouseover="showInfoUser()" onmouseout="hiddeInfoUser()" style="position: absolute; margin: 0em 0.5em;" width="25" height="25" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <circle r="14" transform="matrix(-1 0 0 1 15 15)" stroke="white" stroke-width="2"/>
                            <path d="M15.8408 10.8486V23H13.752V10.8486H15.8408ZM13.5947 7.62549C13.5947 7.28857 13.6958 7.00407 13.8979 6.77197C14.1076 6.53988 14.4146 6.42383 14.8188 6.42383C15.2157 6.42383 15.5189 6.53988 15.7285 6.77197C15.9456 7.00407 16.0542 7.28857 16.0542 7.62549C16.0542 7.94743 15.9456 8.22445 15.7285 8.45654C15.5189 8.68115 15.2157 8.79346 14.8188 8.79346C14.4146 8.79346 14.1076 8.68115 13.8979 8.45654C13.6958 8.22445 13.5947 7.94743 13.5947 7.62549Z" fill="white"/>
                        </svg>
                    </div>

                    <div id="infoUser" style="visibility: hidden; margin: -3em 0em 0em 22em; position: absolute; background-color: rgba(166, 166, 166, 0.17); width: 10em; border-radius: 0.5em; padding: 0.5em;">
                        Escolha um usuário que não contenha espaços.
                    </div>
                </div>

                <div id="senhaRegistro" class="divInputsRegistro" align="center">
                    <p>Senha:</p>
                    <div align="center">
                        <input type="password" name="SenhaUsuario" style="text-align:center;"></input>
                        <svg class="infoPass" onmouseover="showInfoPass()" onmouseout="hiddeInfoPass()" style="position: absolute; margin: 0em 0.5em;" width="25" height="25" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <circle r="14" transform="matrix(-1 0 0 1 15 15)" stroke="white" stroke-width="2"/>
                            <path d="M15.8408 10.8486V23H13.752V10.8486H15.8408ZM13.5947 7.62549C13.5947 7.28857 13.6958 7.00407 13.8979 6.77197C14.1076 6.53988 14.4146 6.42383 14.8188 6.42383C15.2157 6.42383 15.5189 6.53988 15.7285 6.77197C15.9456 7.00407 16.0542 7.28857 16.0542 7.62549C16.0542 7.94743 15.9456 8.22445 15.7285 8.45654C15.5189 8.68115 15.2157 8.79346 14.8188 8.79346C14.4146 8.79346 14.1076 8.68115 13.8979 8.45654C13.6958 8.22445 13.5947 7.94743 13.5947 7.62549Z" fill="white"/>
                        </svg>
                    </div>
                    
                    <div id="infoPass" style="visibility: hidden; margin: -3em 0em 0em 22em; position: absolute; background-color: rgba(166, 166, 166, 0.17); width: 10em; ">
                        Escolha uma senha entre 4 e 8 caracteres.
                    </div>
                </div>

                <div style="align-items: center; margin-top: 2em;">
                    <input id="checkTermos" onclick="termos()" type="checkbox"> Concordo com os <a class="linkTermos" href="LS-Termos.php">termos</a>.
                </div>
        ...
        <?php
            echo"<input name='inputKey' type='hidden' value='".$_POST['inputKey']."'";
        ?>

        <!-- ÁREA DO BOTÃO -->

        <div class="divSeparacao">
            <button class="botaoRegistrar disable" id="botaoReg" type="submit" name="registrar" style="box-shadow: 3px 3px 10px black;" disabled><span style="font-size: 1.8em;">REGISTRAR</span></button>
        </div>
    ...
    <?php
        if (isset($_POST['registrar'])) {
            echo" <input id='inputErroRegistro' type='hidden' name='erro' value='".ValidarRegistro($_POST['NomeUsuario'], $_POST['SenhaUsuario'], $pdo)."'>";
        }
    ?>
