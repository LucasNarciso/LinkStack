...
<head>
    ...
    <script>
        let copiarTexto = () => {
            ...
            if (document.getElementById('divAviso') != null) {
                //console.log("bloqueado");
            }else{
                const url = window.location.href;
                let inputAviso = document.createElement("input");
                inputAviso.value = url;
                document.body.appendChild(inputAviso);
                inputAviso.select();
                document.execCommand('copy');
                document.body.removeChild(inputAviso);
                criarAviso();
            }
        ...
        function criarAviso(){
            ...
            divExterna.setAttribute('class', 'divSeparacao');
            
            body.appendChild(divExterna);

            var divAviso = document.createElement('div');
            divAviso.setAttribute('class', 'divAviso');
            divAviso.setAttribute('id', 'divAviso');

            divAviso.innerHTML = "Link Copiado!";

            divExterna.appendChild(divAviso);

            setTimeout(function exec(){deletarAviso(divExterna)},2000);
        }

        function deletarAviso(divExterna){
            divExterna.classList.toggle('hide');
            setTimeout(function exec1(){document.body.removeChild(divExterna);},1000);
...
<?php
    ...
    $DadosUser = DadosUser($pdo, $usuarioID);
    $NomeLink = NomeLinkUser($pdo, $usuarioID);
    $UrlLink = UrlLinkUser($pdo, $usuarioID);
    $ConfigUser = ConfigUser($pdo, $usuarioID);

    echo "<body id='body' style='background-color:". $ConfigUser[0].";'>";
?>

    <!-- ÁREA DO BOTÃO DE SHARE -->

    <div class="divSeparacao">
        <div class="divBotaoShare">
            <?php
                echo"
                <div onclick='copiarTexto()' href='". "'>
                    <img class='botaoShare' src='Imagens/Share.svg' alt='Botão de compartilhamento'>
                </div>"
                ;
            ?>
        </div>
    ...
    <div class="divSeparacao">
        <?php
        echo "<div class='divFoto' style='border-color: ;'></div>";
        ?>
    ...
    <div class="divSeparacao">
        <?php
            echo "
            <div class='divNomeUser' style='color:" .$ConfigUser[2].";'>
                <p>
                    $DadosUser[1]                
                </p>  
            </div>";
    ...
    <div align="center">
        ...
        <?php
            ...
            for ($i=0; $i < $DadosUser[0]; $i++) {
                ...
                echo "
                <script> var url".$i." = '".$UrlLink[$i]."'</script>";

                if ($ConfigUser[5] == "dupla") {
                    echo "<div class='divSeparacaoLinks' style='width: 85%;'>";
                }else{
                    echo "<div class='divSeparacaoLinks' style='width: 53%;'>";
                }
                
                    
                echo "<div onclick='fun_redirect(url".$i.")' class='card-".$ConfigUser[7]." divLinks'>
                        <div class='divInternaIMG'  style='background-color: ". $ConfigUser[2] . ";'>
    
                        </div>
                        <div class='divInternaLink' style='color: ". $ConfigUser[3] . ";'>
                            <p>".$NomeLink[$i]."</p>";
                            if ($ConfigUser[6] == "sim") {
                                echo $UrlLink[$i];
                            };
                echo    "</div>
                        <div class='divInternaShare' style='background-color: ". $ConfigUser[4] . ";'>
                ...
                if ($i < $DadosUser[0]-1) {
                    $j = $i+1;
                    echo"
                    <script> var url".$j." = '".$UrlLink[$j]."'</script>
                    <div onclick='fun_redirect(url".$j.")' class='card-".$ConfigUser[7]." divLinks'>
                            <div class='divInternaIMG' style='background-color: ". $ConfigUser[2] . ";'>
